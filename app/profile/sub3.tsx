...
if (__DISABLE_HAPTICS_FOR_CRASH_TEST) {
  try {
    ...
    Sentry.addBreadcrumb({ category: 'crash-test', message: 'Haptics disabled in sub3', level: 'info' });
  } catch {}
}
// #endregion
...
export default function Sub3({ scrollY, name, position, goToTab, index, scrollRef, cardRefreshTrigger }: { scrollY: Animated.Value; name: string; position: string; goToTab?: (idx: number) => void; index?: number; scrollRef?: React.RefObject<ScrollView>; cardRefreshTrigger?: number }) {
  ...
  const handleSearchSubmit = async () => {
    ...
    try {
      ...
      console.error('Error searching cards (sub3):', error);
    } finally {
  ...
  const handleGridButtonPress = (buttonText: string) => {
    Sentry.addBreadcrumb({ category: 'nav', message: `Sub3 -> sub4 (${String(buttonText).slice(0, 30)})`, level: 'info' });
    router.push({
      ...
      <View style={styles.container}>
        <View style={styles.searchBarContainer}>
          ...
          <TextInput
            ...
            placeholder={String('Search Affiliation Cards')}
