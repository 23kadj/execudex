...
export default function Legi1({ scrollY, name, position, scrollRef }: Legi1Props) {
  ...
  const handleSearchSubmit = async () => {
    ...
    try {
      ...
      console.error('Error searching cards (legi1):', error);
    } finally {
    ...
    const fetchCardDataFromDB = async () => {
      if (legislationId) {
        try {
          ...
          const shouldShow = await CardGenerationService.checkAgendaButtonVisibility(parseInt(legislationId));
          console.log('Legi1 - Should show generate button:', shouldShow);
  ...
  const handleGenerateCards = async () => {
    ...
    try {
      ...
      const result = await CardGenerationService.generateAgendaCards(ownerId);
      ...
      if (result.success) {
        ...
        if (result.lowMateriality) {
          ...
          const shouldShow = await CardGenerationService.checkAgendaButtonVisibility(ownerId);
          setShowGenerateButton(shouldShow);
  ...
  const renderGrid = () => {
    ...
    if (layoutConfig.gridType === 'soft') {
      // 4-button grid for agenda page (Action, Scope, Process, Exceptions)
      return (
        <View style={styles.softGridContainer}>
          ...
          <View style={styles.gridRow}>
            ...
            <AnimatedPressable
              ...
              onPress={() => handleGridButtonPress('Exceptions')}
              style={[
                styles.gridButton4,
                { transform: [{ scale: gridButton4Scale }] }
              ]}
            >
              <Text style={styles.gridButtonText4}>Exceptions</Text>
            </AnimatedPressable>
          </View>
        </View>
      );
      ...
      return (
        <View style={styles.gridContainer}>
          ...
          <View style={styles.gridRow}>
            ...
            >
              <Text style={styles.gridButtonText5}>More Selections</Text>
            </AnimatedPressable>
            <AnimatedPressable
              onPressIn={() => {
                Haptics.selectionAsync();
            ...
            >
              <Text style={styles.gridButtonText6}>More Selections</Text>
            </AnimatedPressable>
          </View>
      ...
      <View style={styles.container}>
        <View style={styles.searchBarContainer}>
          ...
          <TextInput
            ...
            placeholder={String(`Search ${getScreenDisplayName(getCardIndexScreenForPage('legi1', false))} Cards`)}
