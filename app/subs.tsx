...
export default function Subs() {
  ...
  useEffect(() => {
    ...
    const checkAndEnforceDowngrade = async () => {
      ...
      try {
        ...
        console.error('Error in checkAndEnforceDowngrade:', error);
      }
    };
    ...
    const unregisterSuccess = iapService.onPurchaseSuccess(
      async (purchase) => {
        try {
          console.log('🔵 [IAP] Purchase success handler received (subs)', {
    ...
    const unregisterError = iapService.onPurchaseError(
      async (error) => {
        console.error('❌ [IAP] Purchase error (subs):', error);
    ...
    try {
      ...
      if (bestPurchase) {
        ...
        await iapService.updateUserSubscription(user.id, {
          plan,
          cycle,
          transactionId: bestPurchase.originalTransactionId || bestPurchase.transactionId,
  ...
  const handlePurchaseButtonPress = async () => {
    ...
    try {
      ...
      console.log('🔵 [IAP] Purchase button tapped (subs)', {
        productId,
        userId: user.id,
      ...
      console.error('❌ [IAP] Purchase failed (subs):', error);
      setIsPurchasing(false);
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView 
        ...
        contentContainerStyle={[styles.content, { paddingTop: headerTotalHeight + 12 }]}
        showsVerticalScrollIndicator={false}
        contentInsetAdjustmentBehavior="never"
      >
        {/* Apple Subscription Disclaimer - Replaces Weekly Usage Meter */}
...
const styles = StyleSheet.create({
  ...
  // USAGE BOX (now used for Apple disclaimer text)
  usageBox: {
    width: '100%',
    backgroundColor: '#000',
    borderRadius: 20,
  ...
  headerLeftSpacer: {
    width: 48, // Spacer to keep title centered
  },
  headerIconBtn: {
    padding: 8,
    marginHorizontal: 2,
